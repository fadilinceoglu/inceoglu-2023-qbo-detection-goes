...
```bash
python scripts/reproduce.py --stage figures --figure 3
```

Run the default calculation or `python scripts/reproduce.py --stage analyze`
...
For a faster, non-authoritative preview, set `figures.minute_row_stride` above
`1` in `data/config/paper.toml`; that changes only plotting, not the prepared
one-minute files.
...
`figures/SHA256SUMS` records the checksums of the tracked published files. From
that directory, verify them with `shasum -a 256 -c SHA256SUMS` (macOS) or
`sha256sum -c SHA256SUMS` (GNU coreutils).
